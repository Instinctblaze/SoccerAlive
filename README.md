...
SoccerAlive is an AI-powered sports trajectory simulation and analysis platform. It is a 3D Soccer Visualization and Analysis tool powered by Gemini 3. It uses "SoccerAlive" to analyze player positions in real-time and predict the most "Egoistic" path to the goal.

## Features
- **3D Holographic Pitch**: Full FIFA-standard soccer field rendering.
- **SoccerAlive Analysis**: Gemini 3.0 Pro/Flash predicts future player movements and scoring opportunities.
...
SoccerAlive is set up to deploy as a **single Node app**: the backend serves the built Vite frontend. One service, one URL.
...
```bash
docker build -t socceralive .
docker run -p 3000:3000 -e GEMINI_API_KEY=your_key_here socceralive
```
...
Then add your `GEMINI_API_KEY` to `/home/socceralive/socceralive/.env`.

Check status:
```bash
sudo systemctl status socceralive
sudo journalctl -u socceralive -f
```
...
4. Click **"SoccerAlive"** to see Gemini analyzing the frame!
